...
class TGN(torch.nn.Module):
  ...
  def compute_temporal_embeddings_p(self, source_nodes, destination_nodes, 
                                    source_nodes_final, p_pos_nodes, p_neg_nodes, 
                                    edge_times, edge_times_batch, edge_idxs, n_neighbors=20):
    """
    ...
    """
    # print
    # print('edge_times_batch shape: ', np.array(edge_times_batch).shape)

    NUM_NEG_TRAIN = int(p_neg_nodes.shape[0]/p_pos_nodes.shape[0])

    n_samples = len(source_nodes)
    n_samples_ = len(p_pos_nodes)
    nodes = np.concatenate([source_nodes, destination_nodes, source_nodes_final, p_pos_nodes, p_neg_nodes])
    positives = np.concatenate([source_nodes, destination_nodes])
    edge_times_duplicated = [x for x in edge_times_batch for _ in range(NUM_NEG_TRAIN)]
    timestamps = np.concatenate([edge_times, edge_times, edge_times_batch, edge_times_batch, edge_times_duplicated])

    # print
    # print('edge_times_duplicated shape: ', np.array(edge_times_duplicated).shape)
    ...
    if self.use_memory:
      ...
      # print
      # print('edge times shape: ', edge_times.shape)
      ...
      source_final_time_diffs = torch.LongTensor(edge_times_batch).to(self.device) - last_update[source_nodes_final].long()
      source_final_time_diffs = (source_final_time_diffs - self.mean_time_shift_src) / self.std_time_shift_src
      p_pos_time_diffs = torch.LongTensor(edge_times_batch).to(self.device) - last_update[p_pos_nodes].long()
      ...
      time_diffs = torch.cat([source_time_diffs, destination_time_diffs, source_final_time_diffs, p_pos_time_diffs, p_neg_time_diffs], dim=0)
    ...
    source_final_node_embedding = node_embedding[2 * n_samples : 2 * n_samples + n_samples_]
    p_pos_node_embedding = node_embedding[2 * n_samples + n_samples_ : 2 * n_samples + 2 * n_samples_]
    p_neg_node_embedding = node_embedding[2 * n_samples + 2 * n_samples_:]
    ...
    if self.use_memory:
      ...
      if self.dyrep:
        ...
        source_final_node_embedding = memory[source_nodes_final]
        p_pos_node_embedding = memory[p_pos_nodes]
        p_neg_node_embedding = memory[p_neg_nodes]

    return source_node_embedding, destination_node_embedding, source_final_node_embedding, p_pos_node_embedding, p_neg_node_embedding
