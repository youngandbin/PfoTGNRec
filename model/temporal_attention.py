...
class TemporalAttentionLayer(torch.nn.Module):
  ...
  def forward(self, src_node_features, src_time_features, neighbors_features,
    ...
    query = query.permute([1, 0, 2])  # [1, batch_size, self.query_dim]
    key = key.permute([1, 0, 2])  # [n_neighbors, batch_size, self.key_dim]
    ...
    # If memory dimension and time dimension are not the same, an error will occur at this point.
    ...
    attn_output = attn_output.masked_fill(invalid_neighborhood_mask, 0) # (B, memory dim * 2)
    attn_output_weights = attn_output_weights.masked_fill(invalid_neighborhood_mask, 0)

    # Skip connection with temporal attention over neighborhood and the features of the node itself
    attn_output = self.merger(attn_output, src_node_features) # (B, memory dim)
